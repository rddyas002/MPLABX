...
#define IO_TEMP_KP          0.1
#define IO_TEMP_OMEGA_LEAD  0.364
#define IO_TEMP_OMEGA_LAG   2.74
#define IO_TEMP_OMEGA_TI    0.008
...
void IO_setup(void){
    ...
    // Set USER switch as digital input
    IO_USER_TRIS();
...
void IO_setupPWM_default(void){
    IO_LED_ON = false;
    SetDCOC2PWM(IO_PWM_dc(0));
    SetDCOC3PWM(IO_PWM_dc(0));
    SetDCOC4PWM(IO_PWM_dc(0));
}
...
float IO_temperature_control(float temperature_reference, float measured_temperature, float lower_ctrl_limit, float upper_ctrl_limit, float dt, bool initialise,
            float kp_t, float wi_t, float wz_t, float wp_t){
    static float temperature_error[3] = {0}, temperature_input[3] = {0};
    static float int_input[3] = {0}, int_output_[3] = {0};

    if (initialise){
        memset(&temperature_error[0],0,sizeof(float)*3);
        memset(&temperature_input[0],0,sizeof(float)*3);
        memset(&int_input[0],0,sizeof(float)*3);
        memset(&int_output_[0],0,sizeof(float)*3);
        return 0;
    }

    IO_shiftData(&temperature_error[0]);
    // Negative because of the opposite frame of reference
    temperature_error[0] = (temperature_reference - measured_temperature);
    IO_shiftData(&temperature_input[0]);

    // calculate derivative control signal
    float derivative_signal = kp_t*IO_lead_lag(wz_t, wp_t, dt, &temperature_error[0], &temperature_input[0]);
    float lower_limit_int = lower_ctrl_limit*wi_t;
    float upper_limit_int = upper_ctrl_limit*wi_t;

    return IO_PI_with_limits(1, wi_t, dt, derivative_signal,
...
char IO_getChecksum(char * buffer, char length){
    int i = 0;
    char checksum = 0;
    for (i = 0; i < length; i++){
        checksum ^= *buffer;
...
bool ADC_updated(void){
    return ADC_update;
}

void ADC_update_clear(void){
    ADC_update = false;
}
